#include "include/MicroManager.h"
#include "include/BaseLocationManager.h"
#include "include/Global.h"
#include "include/MapTools.h"
...
void MicroManager::execute(const CoCBot::SquadOrder& inputOrder)
{
	...
	if (CoCBot::Global::Map().getGroundDistance(m_commander->getPosition(), m_units.getPosition()) > 30)
	{
	...
	if (m_order.getType() == CoCBot::SquadOrderTypes::Defend)
	{
		CoCBot::Global::Map().getUnits(nearbyEnemies, m_order.getPosition(), m_order.getRadius(), false, true);

	} // otherwise we want to see everything on the way
	else if (m_order.getType() == CoCBot::SquadOrderTypes::Attack)
	{
		CoCBot::Global::Map().getUnits(nearbyEnemies, m_order.getPosition(), m_order.getRadius(), false, true);
		for (auto& unit : m_units)
		{
			...
			CoCBot::Global::Map().getUnits(nearbyEnemies, unit->getPosition(), m_order.getRadius(), false, true);
		}
	}
	...
	if (m_order.getType() == CoCBot::SquadOrderTypes::Attack || m_order.getType() == CoCBot::SquadOrderTypes::Defend)
	{
...
void MicroManager::regroup(const BWAPI::Position& regroupPosition) const
{
	...
	int regroupDistanceFromBase = CoCBot::Global::Map().getGroundDistance(regroupPosition, ourBasePosition);

	// for each of the units we have
	for (auto& unit : m_units)
	{
		int unitDistanceFromBase = CoCBot::Global::Map().getGroundDistance(unit->getPosition(), ourBasePosition);
...
void MicroManager::tighten(const BWAPI::Position& tightenFormation)
{
	for (auto& unit : m_units)
	{
		Micro::SmartMove(unit, tightenFormation);
	}
}
...
bool MicroManager::unitNearEnemy(BWAPI::Unit unit)
{
	...
	CoCBot::Global::Map().getUnits(enemyNear, unit->getPosition(), 800, false, true);
